...
contract GlobalStorage {
    ...
    function getMyFiles() external view returns (FileInfo[] memory, uint256[] memory) {
        ...
        // If user has no files, return empty arrays
        if (myFileIds.length == 0) {
            return (new FileInfo[](0), new uint256[](0));
        }
        
        // Count valid (non-deleted) files
        ...
        // If no valid files, return empty arrays
        if (validFileCount == 0) {
            return (new FileInfo[](0), new uint256[](0));
        }
        
        FileInfo[] memory myFiles = new FileInfo[](validFileCount);
        uint256[] memory fileIds = new uint256[](validFileCount);
        uint256 currentIndex = 0;
        
        for (uint256 i = 0; i < myFileIds.length; i++) {
            if (myFileIds[i] < globalFiles.length && globalFiles[myFileIds[i]].owner != address(0)) {
                myFiles[currentIndex] = globalFiles[myFileIds[i]];
                fileIds[currentIndex] = myFileIds[i];
                currentIndex++;
            }
        }
